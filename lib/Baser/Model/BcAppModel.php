 ...
 * @param array $options
 * @param bool $deep trueならホストの有効性をチェックする
 * @return bool Success
 */
	public function mbEmail($check, $options = [], $deep = false) {
		...
		$hostName = $pattern[2] . $pattern[3];

		if (function_exists('getmxrr') && getmxrr($hostName, $mxhosts)) {
			return true;
		}
		if (function_exists('checkdnsrr') && checkdnsrr($hostName, 'MX')) {
			return true;
		}
